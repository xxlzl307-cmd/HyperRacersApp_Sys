...
Inherits Serial
		...
		Sub polling(enabled as Boolean)
		  ...
		  case "Aosid 0702"
		    if enabled = true then
		      ...
		      buffer = ""
		      commandmode = "polling"
		      sendstring = ChrB(&h7C) + ChrB(&hFF) + ChrB(&hFF) + ChrB(&h82) + ChrB(&h32) + ChrB(&h00) + ChrB(&hD2)
		      //Set it to polling mode
		      me.Write(sendstring)
	...
	#tag Method, Flags = &h21
		Private Sub ReadAosid0702Data()
		  //Read data from the 0702 by
		  
		  //When the 0702 is in polling mode the data coming back looks like this
		  // CC FF FF 10 32 0D 01 00 10 04 44 6C 8C 00 00 00 00 00 00 96
		  //Bytes 1-5  are the header, always CC FF FF 10 32
		  //Byte 6 and 7 are ?
		  //Bytes 8-19 are the EPC code 00 10 04 44 6C 8C 00 00 00 00 00 00
		  //Byte 20 is the checksum, calculated by summing the previous bytes but rolling over at the byte level and then subtracting the result from &h100
		  ...
		  if processing = false then
		    ...
		    //Make sure we have enough data to consider going forward
		    if InStrB(buffer, Chrb(&hCC)) + 4 > lenb(buffer) then
		      //Not enough data, quite and wait for more
		      processing = false
		      return
		    end if
		    
		    //check to see if we have a valid packet
		    flagStart = InStrB(buffer, Chrb(&hCC) + Chrb(&hFF) + Chrb(&hFF) + Chrb(&h10) + Chrb(&h32))
		    flagEnd = flagStart + 19
		    ...
		    if flagStart + 19 > LenB(buffer) then
		      //Note enough data, return and check back later
		      processing = false
		      return
		    end if
		    
		    //Our packet passed the basic tests so we can process this data now
		    tempUID = midb(buffer, flagStart+7, 12)
		    ...
		    for i as integer = 1 to 12
		      hexPart = hex(asc(midb(tempUID,i,1)))
		      if lenb(hexPart)<2 then
		        hexpart="0"+hexPart
		      end if
		      hexString = hexString + hexPart
	...
	#tag Note, Name = Documentation
		...
		Aosid 0702 is a UHF RFID reader that runs at 9600 baud
	#tag EndNote
	...
	#tag Property, Flags = &h0
		hardwaretype As String = "Trackmate"
	#tag EndProperty
